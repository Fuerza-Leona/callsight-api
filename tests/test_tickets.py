...
def mock_supabase():
    ...
    mock_company_response.data = [
        {"company_id": "5c9d62d2-df16-45b7-ae4a-4b4aaa0c5ae8"}
    ]
    ...
    mock_supabase.table.return_value.select.return_value.eq.return_value.execute.side_effect = [
        ...
        MagicMock(
            data=[
                {
                    "ticket_id": "c7732206-54ac-4d75-b886-9c013802125e",
                    "subject": "Hector prueba 26/04/2025",
                    "company_id": "5c9d62d2-df16-45b7-ae4a-4b4aaa0c5ae8",
                },
                {
                    "ticket_id": "fdc41d83-c48e-450b-a775-38df36a97c40",
                    "subject": "Nuevos cambios",
                    "company_id": "5c9d62d2-df16-45b7-ae4a-4b4aaa0c5ae8",
                },
                {
                    "ticket_id": "22768581-c415-40ac-a50f-9d3a7268bf71",
                    "subject": "Falla en app principal",
                    "company_id": "5c9d62d2-df16-45b7-ae4a-4b4aaa0c5ae8",
                },
            ]
        ),
...
def test_agent_can_create_ticket(mock_supabase):
    ...
    response = client.post(
        f"/api/v1/tickets/companies/{company_id}/tickets", json=ticket_data
    )
...
def test_admin_can_create_ticket(mock_supabase):
    mock_user = SimpleNamespace(
        role="admin", id="admin-123", company_id="admin-company-id"
    )
...
def test_client_can_create_ticket(mock_supabase):
    ...
    mock_user = SimpleNamespace(
        role="client", id="client-456", company_id=client_company_id
    )
    ...
    response = client.post(
        f"/api/v1/tickets/companies/{client_company_id}/tickets", json=ticket_data
    )
    print(response.status_code)
    try:
        print(response.json())
...
def test_create_ticket_missing_information(mock_supabase):
    mock_user = SimpleNamespace(
        role="agent", id="agent-123", company_id="agent-company-id"
    )
    ...
    incomplete_ticket_data = {"description": "Descripción sin asunto", "status": "open"}

    response = client.post(
        f"/api/v1/tickets/companies/{company_id}/tickets", json=incomplete_ticket_data
    )
    print(response.status_code)
    try:
        print(response.json())
...
def test_client_cannot_create_ticket_for_other_company(mock_supabase):
    ...
    mock_user = SimpleNamespace(
        role="client", id="client-789", company_id=client_company_id
    )
    ...
    response = client.post(
        f"/api/v1/tickets/companies/{other_company_id}/tickets", json=ticket_data
    )
    print(response.status_code)
    try:
        print(response.json())
