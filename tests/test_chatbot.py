...
def test_post_chat_db_failure(override_deps, openai_mock):
    ...
    openai_mock.responses.create.side_effect = make_openai_side_effect()
    override_deps.table.return_value.insert.return_value.execute.side_effect = Exception("Connection cannot be established")
...
def test_continue_chat_success(override_deps, openai_mock):
    ...
    resp = client.post(
        "/api/v1/chatbot/continue/abc-123", json={"prompt": "¿Seguro?"}
    )
...
def test_continue_chat_updates_db_and_inserts_msgs(override_deps, openai_mock):
    ...
    assert override_deps.table.return_value.update.called, "last_response_id update missing"
    assert override_deps.table.return_value.insert.call_count == 2, "message inserts missing"
...
def test_get_chat_history_success(override_deps):
    ...
    override_deps.table.side_effect = (
        lambda name: conversation_table if name == "chatbot_conversations" else msg_table
    )
