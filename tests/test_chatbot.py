...
class TestChatEndpoints:
    ...
    async def test_needs_context_with_previous_conversation(self, mock_client):
        ...
        # Assert the correct context was passed
        mock_client.responses.create.assert_called_once()
        call_args = mock_client.responses.create.call_args
        assert call_args[1]["previous_response_id"] == "prev-response-id"
    ...
    @patch("app.api.routes.chatbot.uuid4")
    async def test_post_chat_with_insufficient_context(self, mock_uuid4, mock_needs_context, mock_chat_with_context, mock_client):
    ...
    @patch("app.api.routes.chatbot.client")
    @patch("app.api.routes.chatbot.needs_context", return_value=True)
    async def test_continue_chat_with_insufficient_context(self, mock_needs_context, mock_client, mock_chat_with_context):
    ...
    @patch("app.api.routes.chatbot.needs_context", return_value=True)
    @patch("app.api.routes.chatbot.uuid4")
    async def test_post_chat_with_company_context(self, mock_uuid4, mock_needs_context, mock_chat_with_context, mock_client):
...
Question: Necesito información de los acuerdos de servicio"""
        ...
        assert "como se mencionó durante una llamada previa" in result["response"].lower()
