...
def test_companies(mock_supabase):
    ...
    mock_supabase.table("company_client").select(
        "*"
    ...
    assert response.json()["companies"][0] == {
        "company_id": "test-company-id",
        "name": "Test Company",
        "logo": "test_logo.png",
        "category_id": "test_category",
...
def test_companies_supabase_error(mock_supabase):
    mock_supabase.table("company_client").select("*").execute.side_effect = Exception(
        "Supabase error"
    )
    response = client.get("/api/v1/companies/")
...
def test_company_size_supabase_error(mock_supabase):
    mock_supabase.table("company_client").select(
        "*, users(count)"
    ).execute.side_effect = Exception("Supabase error")
    response = client.get("/api/v1/companies/companySize")
...
def test_users_in_company(mock_supabase):
    mock_company_client = [{"company_id": "test_company_id"}]
    mock_users = [
        {"user_id": "user1", "company_id": "test-company-id"},
        {"user_id": "user2", "company_id": "test-company-id"},
    ]
    mock_supabase.table("company_client").select("company_id").eq(
        "name", "Test Company"
    ).execute.return_value.data = mock_company_client
    mock_supabase.table("users").select("*").eq(
        "company_id", "test_company_id"
    ).execute.return_value.data = mock_users
