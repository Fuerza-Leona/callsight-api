...
def create_monthly_report(
    ...
    elements.append(Paragraph(f"Reporte Mensual:", title_style))
    elements.append(Spacer(1, 12))
    elements.append(Paragraph(f"Para: {company_name}", header_style))
    elements.append(Paragraph(f"Periodo {format_date(start_date)} - {format_date(end_date)}", header_style))
    ...
    if topics_data:
        ...
        for topic in topics_data[:5]:  # Show top 5 topics
            topics_table_data.append([topic.get("topic", ""), str(topic.get("count", 0))])
        ...
        elements.append(Paragraph("No hay datos de temas disponibles para este periodo.", normal_style))
    ...
    if ratings_data:
        ...
        for rating in ratings_data:
            ratings_table_data.append([str(rating.get("rating", "")), str(rating.get("count", 0))])
...
async def save_report_to_storage(
    ...
    storage_path1 = f"reports/{report_id}/{file_name}"
    storage_path = f"{report_id}_{file_name}"
