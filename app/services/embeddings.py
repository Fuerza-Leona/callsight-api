...
async def chat_with_context(
    ...
    for transcript_chunk in (chunk['content'] for chunk in response.data):
        next_chunk = f'\n\nTranscript chunk:\n"""\n{transcript_chunk}\n"""'
        if (
            num_tokens(message + next_chunk, model=GPT_MODEL) > token_budget
        ):
            break
        else:
            message += next_chunk
...
async def get_last_embeddings(
    ...
    '''
    try:
        embeddings_response = supabase.rpc('get_latest_embeddings', params={'user_id': current_user.id}).execute()
